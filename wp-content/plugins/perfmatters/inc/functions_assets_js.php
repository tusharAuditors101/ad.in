...
function perfmatters_assets_js_init() {
	...
	$defer_check = !empty(apply_filters('perfmatters_defer_js', !empty($perfmatters_options['assets']['defer_js']))) && !perfmatters_get_post_meta('perfmatters_exclude_defer_js');

	$delay_check = !empty($perfmatters_options['assets']['delay_js']);
	...
	if($defer_check || $delay_check) {
		...
		if($delay_check && !is_admin() && !perfmatters_is_dynamic_request() && !isset($_GET['perfmatters']) && !perfmatters_is_page_builder() && !is_embed() && !is_feed() && !is_customize_preview()) {
...
function perfmatters_optimize_js($html) {
	...
	if($defer_check) {
		...
		if(empty($perfmatters_options['assets']['defer_jquery'])) {
			array_push($js_exclusions, 'jquery(?:\.min)?.js');
		}

		//add extra user exclusions
		if(!empty($perfmatters_options['assets']['js_exclusions']) && is_array($perfmatters_options['assets']['js_exclusions'])) {
			foreach($perfmatters_options['assets']['js_exclusions'] as $line) {
	...
	foreach($matches[0] as $i => $tag) {
		...
		if(!empty(apply_filters('perfmatters_delay_js', !empty($perfmatters_options['assets']['delay_js'])))) {
			...
			if(empty($perfmatters_options['assets']['delay_js_behavior'])) {
				...
				$delayed_scripts = apply_filters('perfmatters_delayed_scripts', $perfmatters_options['assets']['delay_js_inclusions']);
				...
				if(!empty($perfmatters_options['assets']['delay_js_exclusions']) && is_array($perfmatters_options['assets']['delay_js_exclusions'])) {
					$excluded_scripts = array_merge($excluded_scripts, $perfmatters_options['assets']['delay_js_exclusions']);
				}
			...
			if($delay_flag) {
                ...
                $delayed_tag = sprintf('<script %1$s>', $delayed_atts_string) . (!empty($perfmatters_options['assets']['delay_js_behavior']) ? $matches[3][$i] : '') . '</script>';
...
function perfmatters_print_delay_js() {
	...
	$perfmatters_options = get_option('perfmatters_options');
	$timeout = apply_filters('perfmatters_delay_js_timeout', !empty($perfmatters_options['assets']['delay_timeout']) ? $perfmatters_options['assets']['delay_timeout'] : '');
